...
public class AntColony implements RoutingAlgorithm {
    ...
    private class CalculationContext {
        ...
        double simulateAnts(Configuration config, Path[][] paths){
            ...
            for (int i = 0; i < m; i++) { //runs
              ...
              //  } catch (InterruptedException e){
                 //   e.printStackTrace();
              //  }
                //(new LocalSearch(null,0)).improve_twoOpt(paths,s.best_solution);
                s.save();
           ...
           // pool.shutdown();
            ...
            //System.out.println("Ratio: " + ((double)sum1) / (t2-t1));
            //System.out.println("Prob: " + + ((double)sum2) / (counter2));
        ...
        void globalUpdate(Ant ant, Location[] route, int len, double cost, double [][] trails){
            /*IntStream.range(0,len-1).parallel().forEach((i) -> {
    ...
    protected static class Solution {
        Location[] best_solution = null; //beste gefundene Lösung
